...
public class JiraApplication {
	...
	public static void main(String[] args) {
		...
		String jiraUrl = "https://rb-tracker.bosch.com/tracker08";
        String gitToken = "ODY0MjE5OTYzODYwOoXmZIwoMnkopPdEeaTip8RfCqeS"; // Replace with your Git token
...
/*
        ...
        */
        ...
        String initialUrl = "http://bit.ly/3DhCSII";  // Shortened URL that performs a 302 redirect

        try {
            ResponseEntity<String> response = restTemplate.exchange(initialUrl, HttpMethod.GET, new HttpEntity<>(new HttpHeaders()), String.class);

            if (response.getStatusCode() == HttpStatus.MOVED_PERMANENTLY || response.getStatusCode() == HttpStatus.FOUND) {
                HttpHeaders headers = response.getHeaders();
                String newLocation = headers.getLocation().toString();
                System.out.println("Redirected to: " + newLocation);

                ResponseEntity<String> newResponse = restTemplate.exchange(newLocation, HttpMethod.GET, new HttpEntity<>(new HttpHeaders()), String.class);

                String responseBody = newResponse.getBody();
                System.out.println("Response from new URL: " + responseBody);
            } else {
